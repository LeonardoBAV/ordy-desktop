...
function readEnvValue(appPath: string, key: string): string | undefined {
    if (process.env[key]) {
...
function getPhpHost(appPath: string) {
    return readEnvValue(appPath, 'NATIVEPHP_PHP_SERVER_HOST') || '127.0.0.1';
}

function getRequestedPhpPort(appPath: string) {
    const port = Number.parseInt(readEnvValue(appPath, 'NATIVEPHP_PHP_SERVER_PORT') || '', 10);

    return Number.isInteger(port) && port > 0 ? port : null;
}

function getNightwatchToken(appPath: string) {
    return readEnvValue(appPath, 'NIGHTWATCH_TOKEN');
}
...
async function getPhpPort(appPath: string) {
    const host = getPhpHost(appPath);
    const requestedPort = getRequestedPhpPort(appPath);

    if (requestedPort !== null) {
        if (await canBindToPort(requestedPort, host)) {
            return requestedPort;
        }

        throw new Error(`Could not bind PHP server to ${host}:${requestedPort}`);
    }
...
async function serveApp(secret, apiPort, phpIniSettings): Promise<ProcessResult> {
    ...
    if (nightwatchToken && hasNightwatchInstalled(appPath)) {
        phpNightWatchPort = await findFreePort('127.0.0.1');
    ...
    const phpHost = getPhpHost(appPath);

    console.log('Starting PHP server...');
    const phpPort = await getPhpPort(appPath);
    const phpServer = callPhp(
        ['-S', `${phpHost}:${phpPort}`, serverPath],
        {
            cwd: cwd,
            env,
        },
